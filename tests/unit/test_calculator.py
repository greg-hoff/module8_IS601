...
@pytest.mark.parametrize(
    "a, b, c, expected",
    [
        (2, 3, 5, 10.0),           # Test adding three positive integers
        (-2, -3, -5, -10.0),        # Test adding three negative integers
        (2.5, 3.5, 6.0, 12.0),     # Test adding three positive floats
        (-2.5, 3.5, 1.0, 2.0),    # Test adding a negative float and two positive floats
        (0, 0, 0, 0.0),            # Test adding zeros
    ],
    ids=[
        "add_three_positive_integers",
        "add_three_negative_integers",
        "add_three_positive_floats",
        "add_negative_and_two_positive_floats",
        "add_zeros",
    ]
)
def test_add(a: Number, b: Number, c: Number, expected: Number) -> None:
    """
    Test the 'add' function with various combinations of integers and floats.

    This parameterized test verifies that the 'add' function correctly adds three numbers,
    ...
    Steps:
    1. Call the 'add' function with arguments 'a', 'b', and 'c'.
    ...
    # Assert that the result of add(a, b, c) matches the expected value
    assert result == expected, f"Expected add({a}, {b}, {c}) to be {expected}, but got {result}"
